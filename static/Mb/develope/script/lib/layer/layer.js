...
//点触事件
  ...
  'function' == typeof define ? define(function() {
    ...
    document.head.appendChild(function(){
      ...
      link.href = path + 'lib/layer/need/layer.css?2.0';
      link.type = 'text/css';
      link.rel = 'styleSheet'
      link.id = 'layermcss';
      return link;
    ...
    var js = document.scripts,script, jsPath,path;
    for(var i = 0 ; i < js.length ;i++ ){
      if(js[i].src && js[i].src.indexOf('script/require.js')>0 ){
        script = js[i];
        jsPath = script.src;
        path = jsPath.substring(0, jsPath.lastIndexOf("/") + 1);
        path = path + 'lib/layer/need/layer.css?2.0';
        break;
      }
      if(js[i].src && js[i].src.indexOf('script/lib/layer/layer.js')>0 ){
        script = js[i];
        jsPath = script.src;
        path = jsPath.substring(0, jsPath.lastIndexOf("/") + 1);
        path = path + 'need/layer.css?2.0';
        break;
      }
    }
