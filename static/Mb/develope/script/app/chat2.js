...
define([
    ...
    return {
        ...
        liveList:function () {
            ...
            if( localDataCache.getPageCacheData('back') ){
                isTop = true;
                top = -50;
                chatHeader.css('top', top);
                chatHeaderCommonHide.remove();
                box.html(localDataCache.getPageCacheData('lidata'));
                ...
                if( /*(msg.type==3 && window.location.href.indexOf('mb/live/LiveList')>0 ) ||*/
                    ...
                    for ( var k in window.liveTag){
                        for ( var h in tag ){
                            ...
                            if( window.liveTag[k].id == tag[h].id ){
        ...
        countdownRun:function () {
            ...
            var runTimerFunc = function() {
                ...
                for(var i=delectIndex.length;i>0;i--){
                    countdownArray.splice(delectIndex[i] + 1,1);
                }
                chushiHUa = true;
                delectIndex=[];
                millisecond--;
