...
define(['require', 'jquery', 'socket.io'], function (require, $, io) {
	...
	function _pushNotification(userssid) {
		...
		// 连接服务端
		var url = 'http://io.jiguo.com:2126';
		//检测是否是测试环境
		for (var i = 0; i < document.scripts.length; i++) {
			var scriptSrc = document.scripts[i].src;
			if (scriptSrc.indexOf('/script/require.js') > -1) {
				if (scriptSrc.indexOf('/static/Mb/develope/script/require.js') > -1) {
					url = 'http://msg.jiguo.com:2126';
					break;
				}
			}
		}
