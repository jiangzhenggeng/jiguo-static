...
(function($){
	$.pageRuler = function(params){
		...
		//参考线的水平拖移
		$('#zxxScaleBox').on("mousedown",".zxxRefLine_v", function(){
			oDrag = $(this);	
			dragFlag = true;
			f.dashv();
		});
		//参考线的垂直拖移
		$('#zxxScaleBox').on("mousedown",'.zxxRefLine_h', function(){
			oDrag = $(this);	
			dragFlag = true;
			f.dashh();
		});
	...
	if ( typeof define === "function" && define.amd ) {
		define( "pageRuler", [], function() {} );
	}

	var js = document.scripts,
		script;
	for(var i in js){
		if(js[i].src.indexOf('/js/jquery.zxxPageRuler.1.1.js')!=-1){
			script = js[i];
			break;
		}
	}
	var jsPath = script.src;
	var cssPAth = jsPath.substr(0,jsPath.length - ('/js/jquery.zxxPageRuler.1.1.js').length)+'/css/ruler.css';
