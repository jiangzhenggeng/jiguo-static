...
//图片裁剪
function cropperImage(dom, options, callback) {

	var croppers = null;

	function _init(dom, options, callback) {
			...
			callback = callback || function (replayData) {
				...
				if(options.aspectType==2){
					$(this).closest('li').find('input[type="hidden"]').val( 'http://s1.jiguo.com/' + item.fileid + '/640' );
				}else{
			...
			if( options.boxHeight > $(window).height() ){
				options.boxHeight = $(window).height();
			}
			if (!dom) return;
            ...
            <div class="note-warp event-page-notice" id="warp-' + _id + '">\
            	<div style="flex: 1;width: 500px;height: 500px;"><img id="' + _id + '" src="' + src + '"/></div>\
					...
					success: function () {
						createCropper($('#' + _id), options, callback, {
							preview: preview
						});
					}
				});
	...
	function createCropper(dom, options, callback, bop) {
