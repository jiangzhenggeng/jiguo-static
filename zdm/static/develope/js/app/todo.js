...
//                    查用户
    ...
    function chooseUser() {
        $('body').off('click', '[data-choose-author]').on('click', '[data-choose-author]', function () {
            ...
            if ($(this).data('data-choose-author') == 'send') {
                layer.msg('该用户已添加');
                return false;
            }
            ...
            $(this).data('data-choose-author', 'send');
    ...
    function remarksUser(orderid, val) {
        layer.prompt({
            title: '备注',
            value: val
        }, function (text, index) {
            ...
            common.ajax('post', '/admin2/work/EditOrderRemarks', data, 'json', function () {
                layer.close(index);
                layer.msg('操作成功', function () {
                    window.location.reload();
                })
