...
//关闭
function close(doc) {
    $('.report-box .close').on('click', function () {
        doc.find('input').val('');
...
function submit(target) {
    ...
    $('' + target + ' button[data-submit]').on('click', function () {
        ...
        if (name && nameValue.length !== 0) {
            ...
            name.siblings('.err').removeClass('opacity').text('请输入姓名');
            return false;
        }
        if (tel && telValue.length !== 0) {
            ...
            tel.siblings('.err').removeClass('opacity').text('请输入手机');
            return false;
        }
        if ( weixinValue.length == 0) {
            weixin.siblings('.err').removeClass('opacity').text('请输入微信');
            return false;
        }
        if (email && emailValue.length !== 0) {
            ...
            email.siblings('.err').removeClass('opacity').text('请输入邮箱');
            return false;
        }
        ...
        if (company && companyValue.length !== 0) {
            ...
            company.siblings('.err').removeClass('opacity').text('请输入公司');
            return false;
        ...
        if (place && placeValue.length !== 0) {
            ...
            place.siblings('.err').removeClass('opacity').text('请输入职位');
            return false;
        }
        if (target == '#form-part') {
