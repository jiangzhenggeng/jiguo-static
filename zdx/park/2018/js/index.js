$(function () {
    ...
    $('head').append('<style>.header{height:' + windowHeight + 'px;width:100%;position: relative;color:#fff;background-image: url(http://cdn.jiguo.com/zdx/park/2018/img/bg.jpg);background-repeat: no-repeat;background-size: 100% 100%}</style>');

    if (windowWidth < 768) {
        $('.navbar-header').css({'backgroundColor':'#fff'});
        $('.video-box iframe').attr({height:'360px'});
    }
    if (windowWidth < 640) {
        var width = windowWidth-20;
        var height = ((width/4)*3)<360?((width/4)*3):360;
        $('.video-box iframe').attr({src:"http://v.qq.com/iframe/player.html?vid=k0382j8w0gd&width="+width+"&height="+height+"&auto=0",width:width,height:height});
    }
...
//手机导航栏
    ...
    $('#new_year').flipcountdown({
        size:'sm',
        beforeDateTime:'3/8/2018 09:00:00'
    });
    //视频播放
    playVideo();
    //轮播图
...
function changeNav() {
    var nav = $('.nav'),
        ...
        reviewScrollTop = $('#review').offset().top - 380,
...
//滚动事件
    $(window).scroll(function () {
        ...
        if (0 <= scrollTop && scrollTop < showScrollTop) {
            changeActive($('#first'))
        } else if (showScrollTop <= scrollTop && scrollTop < reviewScrollTop) {
            changeActive($('#second'))
        } else if (reviewScrollTop <= scrollTop && scrollTop < enrollScrollTop) {
            changeActive($('#third'))
